...
func main() {
	configPath := configFilePath("config.yaml")

	var flatCfg ConfigFlat

	if _, err := konform.Load(&flatCfg, konform.FromYAMLFile(configPath)); err != nil {
		log.Fatal(err)
	}
	...
	if _, err := konform.Load(&bytesCfg, konform.FromYAMLBytes(yamlBytes)); err != nil {
		log.Fatal(err)
	}
	...
	if _, err := konform.Load(&inlineNestedCfg, konform.FromYAMLFile(configPath)); err != nil {
		log.Fatal(err)
	}
	...
	if _, err := konform.Load(&nestedCfg, konform.FromYAMLFile(configPath)); err != nil {
		log.Fatal(err)
	}
