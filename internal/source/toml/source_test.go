...
func TestNewFileSource(t *testing.T) {
	got := NewFileSource("config.toml", "/tmp", common.UnknownKeySuggestionError)
	if got.path != "config.toml" || got.callerDir != "/tmp" {
...
func TestLoadFile(t *testing.T) {
	...
	tests := []struct {
		...
		}, wantErrType: errs.DecodeSourceField, wantErrLike: []string{`toml "mode" -> Mode`, "expected string, got bool"}},
	}
